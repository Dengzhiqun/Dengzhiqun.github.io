var openid; 
requestByGET("http://dwitf.applinzi.com/number/php/get_openid.php",function(res){
	openid = res;
});
...
function show_ranking_div(){
	...
	$url = "http://dwitf.applinzi.com/number/php/get_ranking_list.php";
	...
	var countNumberTimer = setInterval(function(){
		if (beginCount) {
			if(countNumber > 0){
				...
				requestByGET("http://dwitf.applinzi.com/number/php/update_score.php?openid="+openid+"&score="+number ,function(score){
                	var high_score_span = getElementByID("highScore");
                    high_score_span.innerHTML = score;

                    requestByGET("http://dwitf.applinzi.com/number/php/get_ranking.php?score="+score ,function(res){
